...
Enhanced version with cost tracking and streaming support
"""
...
# Calculate cached content size
CACHED_TOKENS_SIZE = len(FULL_PROMPT.split()) * 1.3
...
def format_cost_report(session, input_tokens, output_tokens, cost_info, is_first_message):
    ...
    report += f"- **💵 Total: ${cost_info['total_cost']:.6f}**\n\n"
    ...
    report += f"**📊 Session Stats:** {session['message_count']} messages | "
    report += f"${total_with_storage:.6f} total"
...
def chat():
    ...
    def generate():
        try:
            ...
            # Calculate costs
            question_tokens = len(user_message.split()) * 1.3
...
if __name__ == '__main__':
    print("=" * 60)
    print("🚀 AI Teaching Assistant API (Enhanced)")
    print("=" * 60)
    print(f"📚 Model: {MODEL}")
